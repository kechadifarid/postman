...
import { ReporterRuntime, createDefaultWriter, getEnvironmentLabels, getFallbackTestCaseIdLabel, getFrameworkLabel, getHostLabel, getLanguageLabel, getProjectName, getSuiteLabels, getThreadLabel, md5 } from "allure-js-commons/sdk/reporter";
...
class AllureReporter {
  ...
  onBeforeItem(err, args) {
    ...
    var titlePath = projectName ? [projectName, ...testPath] : testPath;
    var hostLabel = getHostLabel();
    var threadLabel = getThreadLabel();
    var {
      labels,
      links
    } = extractMeta(args.item.events);
    var legacyFullName = "".concat(testPath.join("/"), "#").concat(item.name);
    this.currentTest = this.allureRuntime.startTest({
      name: args.item.name,
      fullName,
      titlePath,
      stage: Stage.RUNNING,
      labels: [getLanguageLabel(), getFrameworkLabel("newman"), hostLabel, threadLabel, getFallbackTestCaseIdLabel(md5(legacyFullName)), ...labels, ...getEnvironmentLabels()],
      links
    });
...
function _getFullName(item) {
  ...
  var projectName = getProjectName();
  var fullNameBase = projectName ? "".concat(projectName, ":").concat(chain.join("/")) : chain.join("/");
  return "".concat(fullNameBase, "#").concat(item.name);
}
